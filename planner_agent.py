...
strategy: one short English sentence explaining the whole policy.
...
right_clear -> RIGHT
all_blocked -> STOP
Put the three goal rules first. Choose a deterministic order for the three
clear-direction fallback rules next. Put all_blocked last.
stop_condition must be the string all_blocked.
The fallback order is your design choice, not a new customer requirement.
